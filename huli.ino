...
// --- Smith预估器 ---
#define SMITH_DELAY_STEPS  12        // τ/dt = 1.24s / 0.1s = 12步
#define SMITH_T            1.64f     // 系统时间常数(s)
#define SMITH_DT           0.1f      // 采样周期(s)
// 一阶模型离散系数: y_pred[k] = a*y_pred[k-1] + b*u[k-1]
// a = e^(-dt/T), b = K*(1-a), K=27.2(归一化), u∈[0,1]
#define SMITH_A  0.9408f             // e^(-0.1/1.64)
#define SMITH_B  1.6100f             // 27.2 * (1 - 0.9408) / 1.0  注：u已归一化
float smithPredBuffer[SMITH_DELAY_STEPS + 1] = {0};  // 环形缓冲区
int   smithBufHead = 0;              // 写入头指针
float smithYpred   = 0.0f;           // 当前预估输出（含滞后补偿）
float lastPowerNorm = 0.0f;          // 上一拍归一化功率，供Smith递推使用
...
void loop() {
  ...
  if (heaterPendingStart) {
    if (millis() - heaterStartDelayTime >= 1000) {
      ...
      lastPidTime   = millis();
      // 重置Smith预估器
      memset(smithPredBuffer, 0, sizeof(smithPredBuffer));
      smithBufHead  = 0;
      smithYpred    = 0.0f;
      lastPowerNorm = 0.0f;
...
// ================= 12. 模糊自适应PID查表与双线性插值 =================
// 网格定义：e in [-16,16] 13点，ec in [-4,4] 13点
// 行索引对应 e，列索引对应 ec
// 数值为 MATLAB evalfis 输出的模糊增益（未乘比例因子）
// TODO: 运行 MATLAB 脚本后将下方占位数据替换为实际导出值

static const float Kp_LUT[13][13] = {
    {2.8398f, 2.8056f, 2.8398f, 2.5660f, 2.4988f, 2.4986f, 2.4988f, 2.2497f, 1.9995f, 1.7499f, 1.4997f, 1.4996f, 1.4997f},  // e[0] = -16.0000
    {2.8056f, 2.8056f, 2.8056f, 2.5660f, 2.4981f, 2.2496f, 2.2496f, 2.2496f, 2.0000f, 1.7483f, 1.4428f, 1.2490f, 1.2490f},  // e[1] = -13.3333
    {2.8396f, 2.8052f, 2.8396f, 2.5660f, 2.4984f, 2.2495f, 2.0425f, 2.0554f, 1.9991f, 1.7483f, 1.4562f, 1.2489f, 1.0430f},  // e[2] = -10.6667
    {2.5659f, 2.5659f, 2.5659f, 2.5659f, 2.4981f, 2.2495f, 2.0014f, 1.7515f, 1.7501f, 1.4992f, 1.2489f, 1.2489f, 1.0016f},  // e[3] = -8.0000
    {2.4988f, 2.4986f, 2.4988f, 2.4986f, 2.4984f, 2.2495f, 1.9992f, 1.7500f, 1.5001f, 1.2491f, 1.0000f, 0.9997f, 1.0000f},  // e[4] = -5.3333
    {2.4986f, 2.4986f, 2.4986f, 2.2496f, 2.2492f, 2.0000f, 1.7499f, 1.4993f, 1.2494f, 0.9994f, 0.7501f, 0.7501f, 0.7501f},  // e[5] = -2.6667
    {2.4988f, 2.4986f, 2.4988f, 2.2496f, 1.9988f, 1.7498f, 1.4997f, 1.2491f, 1.0003f, 0.7501f, 0.5004f, 0.5007f, 0.5004f},  // e[6] = 0.0000
    {2.2496f, 2.2496f, 2.2496f, 2.0000f, 1.7495f, 1.4992f, 1.2490f, 1.2490f, 1.0001f, 0.7501f, 0.5007f, 0.5007f, 0.5007f},  // e[7] = 2.6667
    {1.9992f, 1.9995f, 1.9991f, 1.7498f, 1.4992f, 1.2489f, 0.9999f, 0.9996f, 0.9999f, 0.7501f, 0.5004f, 0.5007f, 0.5004f},  // e[8] = 5.3333
    {1.9976f, 1.7500f, 1.7500f, 1.4980f, 1.2477f, 0.9994f, 0.7502f, 0.7502f, 0.7502f, 0.7500f, 0.4981f, 0.4335f, 0.4336f},  // e[9] = 8.0000
    {1.9561f, 1.7500f, 1.5431f, 1.2039f, 0.9573f, 0.7505f, 0.5008f, 0.5013f, 0.5008f, 0.4987f, 0.4984f, 0.4334f, 0.1604f},  // e[10] = 10.6667
    {1.7499f, 1.7499f, 1.5564f, 1.0009f, 0.7501f, 0.7500f, 0.5007f, 0.5007f, 0.5007f, 0.4335f, 0.4335f, 0.4334f, 0.1944f},  // e[11] = 13.3333
    {1.4997f, 1.4996f, 1.4997f, 0.9992f, 0.5004f, 0.5007f, 0.5004f, 0.5007f, 0.5004f, 0.4335f, 0.1602f, 0.1944f, 0.1602f}   // e[12] = 16.0000
};

static const float Ki_LUT[13][13] = {
    {0.2136f, 0.2593f, 0.2136f, 0.5774f, 0.6669f, 0.6671f, 0.6669f, 0.9998f, 1.3325f, 1.6659f, 1.9994f, 1.9993f, 1.9994f},  // e[0] = -16.0000
    {0.2593f, 0.2593f, 0.2593f, 0.5774f, 0.6677f, 1.0000f, 1.0000f, 1.0000f, 1.3322f, 1.6659f, 1.9993f, 1.9993f, 1.9993f},  // e[1] = -13.3333
    {0.2136f, 0.2597f, 0.2139f, 0.5781f, 0.6675f, 1.0001f, 1.2755f, 1.2585f, 1.3332f, 1.6665f, 2.0000f, 2.0001f, 2.0000f},  // e[2] = -10.6667
    {0.2593f, 0.5773f, 0.5775f, 0.9090f, 1.0001f, 1.0001f, 1.3303f, 1.6637f, 1.6655f, 1.9994f, 2.3328f, 2.3328f, 2.3328f},  // e[3] = -8.0000
    {0.2136f, 0.5773f, 0.6643f, 0.9999f, 1.3329f, 1.3327f, 1.3330f, 1.6657f, 1.9988f, 2.3326f, 2.6659f, 2.6662f, 2.6659f},  // e[4] = -5.3333
    {0.5774f, 0.5774f, 0.6636f, 0.9999f, 1.3334f, 1.6658f, 1.6658f, 1.9992f, 2.3321f, 2.6661f, 2.9996f, 2.9996f, 2.9996f},  // e[5] = -2.6667
    {0.6669f, 0.6671f, 0.6669f, 1.0000f, 1.3335f, 1.6660f, 1.9994f, 2.3325f, 2.6654f, 2.9996f, 3.3320f, 3.3319f, 3.3320f},  // e[6] = 0.0000
    {0.6671f, 0.9999f, 1.0000f, 1.3331f, 1.6664f, 1.9994f, 2.3327f, 2.3327f, 2.6656f, 2.9997f, 3.3353f, 3.4217f, 3.4217f},  // e[7] = 2.6667
    {0.6670f, 0.9999f, 1.2755f, 1.6639f, 1.9998f, 2.3328f, 2.6659f, 2.6663f, 2.6660f, 2.9997f, 3.3346f, 3.4218f, 3.7864f},  // e[8] = 5.3333
    {1.3327f, 1.3328f, 1.6032f, 1.9974f, 2.3330f, 2.3348f, 2.6687f, 2.9995f, 2.9995f, 3.0908f, 3.4216f, 3.4218f, 3.7407f},  // e[9] = 8.0000
    {1.9977f, 1.9971f, 1.9977f, 2.3310f, 2.6657f, 2.7408f, 2.7236f, 2.9995f, 3.3315f, 3.4210f, 3.7861f, 3.7403f, 3.7864f},  // e[10] = 10.6667
    {1.9993f, 1.9993f, 1.9993f, 2.3327f, 2.6669f, 2.9996f, 2.9996f, 2.9996f, 3.3312f, 3.4217f, 3.7407f, 3.7407f, 3.7407f},  // e[11] = 13.3333
    {1.9994f, 1.9993f, 1.9994f, 2.3327f, 2.6664f, 2.9998f, 3.3320f, 3.3319f, 3.3320f, 3.4217f, 3.7864f, 3.7407f, 3.7864f}   // e[12] = 16.0000
};

static const float Kd_LUT[13][13] = {
    {0.6667f, 0.5001f, 0.3694f, 0.2434f, 0.0527f, 0.0640f, 0.0527f, 0.0640f, 0.0527f, 0.5715f, 0.8189f, 0.7500f, 0.6667f},  // e[0] = -16.0000
    {0.6668f, 0.5001f, 0.3796f, 0.2434f, 0.0641f, 0.1444f, 0.1444f, 0.1444f, 0.1444f, 0.4398f, 0.5831f, 0.5832f, 0.5834f},  // e[1] = -13.3333
    {0.6666f, 0.5001f, 0.3694f, 0.2434f, 0.0528f, 0.1444f, 0.1661f, 0.1661f, 0.1663f, 0.3011f, 0.3983f, 0.4540f, 0.5144f},  // e[2] = -10.6667
    {0.5834f, 0.5000f, 0.3802f, 0.2285f, 0.1444f, 0.1444f, 0.1668f, 0.2499f, 0.2500f, 0.2523f, 0.3543f, 0.4179f, 0.5007f},  // e[3] = -8.0000
    {0.5000f, 0.4166f, 0.3477f, 0.2506f, 0.1668f, 0.1668f, 0.1667f, 0.2500f, 0.3332f, 0.3338f, 0.3477f, 0.4166f, 0.5000f},  // e[4] = -5.3333
    {0.5000f, 0.4166f, 0.3521f, 0.2505f, 0.2500f, 0.2500f, 0.2500f, 0.2500f, 0.3330f, 0.3338f, 0.3521f, 0.4166f, 0.5000f},  // e[5] = -2.6667
    {0.5000f, 0.4166f, 0.3477f, 0.3338f, 0.3333f, 0.3332f, 0.3333f, 0.3332f, 0.3333f, 0.3338f, 0.3477f, 0.4166f, 0.5000f},  // e[6] = 0.0000
    {0.5000f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.4166f, 0.5000f},  // e[7] = 2.6667
    {0.5000f, 0.5000f, 0.5000f, 0.5000f, 0.5000f, 0.5000f, 0.5000f, 0.5000f, 0.5000f, 0.5001f, 0.5000f, 0.5001f, 0.5000f},  // e[8] = 5.3333
    {0.6494f, 0.5405f, 0.4441f, 0.5005f, 0.5839f, 0.5839f, 0.5839f, 0.5839f, 0.5835f, 0.5839f, 0.6001f, 0.6667f, 0.6494f},  // e[9] = 8.0000
    {0.9456f, 0.5485f, 0.3896f, 0.5165f, 0.6810f, 0.6854f, 0.6810f, 0.6854f, 0.6667f, 0.6673f, 0.6821f, 0.7585f, 0.9456f},  // e[10] = 10.6667
    {0.9352f, 0.6168f, 0.5842f, 0.5936f, 0.7500f, 0.7500f, 0.7500f, 0.7500f, 0.6670f, 0.6675f, 0.6870f, 0.7586f, 0.9352f},  // e[11] = 13.3333
    {0.9466f, 0.8556f, 0.8339f, 0.8332f, 0.8333f, 0.8332f, 0.8333f, 0.7500f, 0.6669f, 0.6675f, 0.6822f, 0.7586f, 0.9466f}   // e[12] = 16.0000
};

// 双线性插值查表：输入物理量 e, ec，输出实际 Kp, Ki, Kd
// e  网格：13点，步长 = 32/12，范围 [-16, 16]
// ec 网格：13点，步长 = 8/12， 范围 [-4,  4]
void queryFuzzyPID(float e, float ec, float *kp, float *ki, float *kd) {
  // 限幅
  e  = constrain(e,  -16.0f, 16.0f);
  ec = constrain(ec,  -4.0f,  4.0f);

  // 归一化到 [0, 12] 区间内的连续坐标
  float pos_e  = (e  + 16.0f) * 12.0f / 32.0f;
  float pos_ec = (ec +  4.0f) * 12.0f /  8.0f;

  // 左端点索引（钳位到 [0, 11]，保证 idx+1 不越界）
  int ie  = constrain((int)pos_e,  0, 11);
  int iec = constrain((int)pos_ec, 0, 11);

  // 区间内小数部分（双线性权重）
  float te  = pos_e  - ie;
  float tec = pos_ec - iec;

  // 四个角点权重
  float w00 = (1.0f - te) * (1.0f - tec);
  float w01 = (1.0f - te) * tec;
  float w10 = te           * (1.0f - tec);
  float w11 = te           * tec;

  // 双线性插值并乘以比例因子
  *kp = (Kp_LUT[ie][iec]*w00 + Kp_LUT[ie][iec+1]*w01 +
         Kp_LUT[ie+1][iec]*w10 + Kp_LUT[ie+1][iec+1]*w11) * PID_KP;
  *ki = (Ki_LUT[ie][iec]*w00 + Ki_LUT[ie][iec+1]*w01 +
         Ki_LUT[ie+1][iec]*w10 + Ki_LUT[ie+1][iec+1]*w11) * PID_KI;
  *kd = (Kd_LUT[ie][iec]*w00 + Kd_LUT[ie][iec+1]*w01 +
         Kd_LUT[ie+1][iec]*w10 + Kd_LUT[ie+1][iec+1]*w11) * PID_KD;
...
void runPidTask() {
  ...
  // === Smith预估器 ===
  // 1. 取出τ步前的预估值（即滞后τ后模型的输出）
  int delayedIdx = (smithBufHead + 1) % (SMITH_DELAY_STEPS + 1);
  float smithYdelayed = smithPredBuffer[delayedIdx];

  // 2. 用一阶模型递推当前预估值（u已归一化到[0,1]）
  float u_normalized = (heaterDelayMicros > 0 && heaterTriggerEnabled)
                       ? (float)getHeaterDelay(50.0f) / (float)heaterDelayMicros  // 近似当前功率归一化
                       : 0.0f;
  smithYpred = SMITH_A * smithYpred + SMITH_B * lastPowerNorm;

  // 3. 写入环形缓冲区
  smithPredBuffer[smithBufHead] = smithYpred;
  smithBufHead = (smithBufHead + 1) % (SMITH_DELAY_STEPS + 1);

  // 4. Smith补偿误差：用(预估值 - 滞后预估值)修正实际反馈
  //    等效无滞后误差 = 设定值 - [实测温度 + (预估 - 滞后预估)]
  float smithComp = smithYpred - smithYdelayed;  // 预估的滞后补偿量
  float error = PID_SETPOINT - (filteredTemperature + smithComp);

  float ec = (error - pidLastError) / dt;
  ec = constrain(ec, -4.0f, 4.0f);  // 限幅，与queryFuzzyPID输入范围一致
  pidLastError = error;

  // 模糊自适应查表，动态更新 PID 参数
  float current_Kp, current_Ki, current_Kd;
  queryFuzzyPID(error, ec, &current_Kp, &current_Ki, &current_Kd);

  pidIntegral += error * dt;
  pidIntegral  = constrain(pidIntegral, -50.0f, 50.0f);

  float output = current_Kp * error + current_Ki * pidIntegral + current_Kd * ec;

  float power = constrain(output, 0.0f, 95.0f);
  lastPowerNorm = power / 95.0f;  // 记录归一化功率供下次Smith递推
  if (power < 5.0f) {
    heaterTriggerEnabled = false;
    heaterDelayMicros = 0;
  } else {
